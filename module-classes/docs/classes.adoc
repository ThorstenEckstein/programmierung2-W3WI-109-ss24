...
Jedes Projekt bzw. `Modul` hat eine ähnliche Struktur ("_Convention over Configuration_"), das erleichtert auch einfach die Nutzung und Wiedererkennung:
      ...
      {dir-icon-src} main
        {dir-icon-src} java
          {dir-icon-src} de
            {dir-icon-src} dhbw
              {dir-icon-src} demo
              {dir-icon-src} exercise
              {dir-icon-src} solution
      {dir-icon-test} test
    {dir-icon} target
----
