...
. Eine Schnittstelle `Zug` mit den Schnittstellenmethoden
...
Die wichtigsten Merkmale von Abstraktionen in Form von `abstract classes`:

Das *Schlüsselwort* ("abstrakter Modifikator") zur Umsetzung von Abstraktionen in Java ist

 abstract

* Eine abstrakte Klasse enthält den abstrakten *Modifikator* in der Klassensignatur
...
.. Das Feld `order` soll dabei möglichst stark geschützt sein
. Zwei konkrete Klassen, die beide die abstrakte Klasse _beerben_, nämlich
.. `PassengerWagon` und
.. `FreightWagon`
