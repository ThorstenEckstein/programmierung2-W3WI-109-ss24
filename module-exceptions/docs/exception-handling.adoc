...
Grundsätzlich stellt Java einen sogenannten `exception handler` bereit, und zwar einfach mithilfe des `catch` Abschnittes. Hier sollte der abgefangene Fehler "behandelt" werden, wie folgt **beispielhaft** umgesetzt:

[,java, lines]
----
try {
  String file = "document.docx";
  bollean loaded = fileHandler.load(file);
...
. *Wirf* diese neue Exception einfach mittels `throws` in der konkreten Methode `execute()` deiner konkreten Klasse.
