...
[start=1]
. Nach dem Start von IntelliJ die Menüoption `Git > clone ...` aufrufen:

image::images/intellij-git-menu-clone.png[width="25%", align="center"]

[start=2]
. Dann erscheint der folgende `clone`-*Dialog*:

Nutze dazu die folgenden Angaben:
...
[start=4]
. Dann fehlt nur noch das *Öffnen in einem separaten* Fenster -> `[New Window]`:
...
[%collapsible, title="Aufklappen für die `git` Kommandos ..."]
====
